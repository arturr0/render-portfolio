...
//   );
  ...
  $(document).ready(function () {
    const animDivs = document.querySelectorAll('.content'); // NodeList (multiple elements)
    const main = document.querySelectorAll('.main'); // NodeList (multiple elements)
    const scrollBox = document.getElementById('homeContentPojects');
    //const main = document.getElementById('homeContentPojects');
...
scrollBox.addEventListener('scroll', () => {
  ...
  animDivs.forEach(animDiv => {
    if (scrollBox.scrollTop > 0) {
      animDiv.classList.add('collapsed');
      main.classList.add('full');
    } else {
      animDiv.classList.remove('collapsed');
